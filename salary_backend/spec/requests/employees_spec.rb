require 'rails_helper'

RSpec.describe "Employees API", type: :request do
  describe "POST /employees" do
    it "creates an employee" do
      post "/employees", params: {
        employee: {
          full_name: "Rahul Sharma",
          job_title: "Engineer",
          country: "India",
          salary: 50000
        }
      }

      expect(response).to have_http_status(:created)
    end

    it "returns 400 for invalid employee" do
      post "/employees", params: {
        employee: { full_name: "", salary: -10 }
      }

      expect(response).to have_http_status(:bad_request)
    end
  end

  describe "GET /employees" do
    it "returns list of employees" do
      Employee.create!(full_name: "Jane Doe", job_title: "HR", country: "India", salary: 40000)

      get "/employees"

      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)["data"].length).to eq(1)
    end
  end

  describe "PUT /employees/:id" do
    it "updates an employee" do
      emp = Employee.create!(full_name: "A", job_title: "Engineer", country: "India", salary: 10000)

      put "/employees/#{emp.id}", params: {
        employee: { salary: 20000 }
      }

      expect(response).to have_http_status(:ok)
      expect(emp.reload.salary).to eq(20000)
    end

    it "returns 400 for invalid update" do
      emp = Employee.create!(full_name: "A", job_title: "Engineer", country: "India", salary: 1000)

      put "/employees/#{emp.id}", params: {
        employee: { salary: -50 }
      }

      expect(response).to have_http_status(:bad_request)
    end

    it "returns 404 if employee not found" do
      put "/employees/999999", params: {
        employee: { full_name: "Test" }
      }

      expect(response).to have_http_status(:not_found)
    end
  end

  describe "DELETE /employees/:id" do
    it "deletes employee" do
      emp = Employee.create!(full_name: "A", job_title: "Engineer", country: "India", salary: 10000)

      delete "/employees/#{emp.id}"

      expect(response).to have_http_status(:no_content)
      expect(Employee.exists?(emp.id)).to be false
    end

    it "returns 404 when deleting non-existent employee" do
      delete "/employees/999999"

      expect(response).to have_http_status(:not_found)
    end
  end

  describe "GET /employees/:id" do
    it "returns 404 for non-existent employee" do
      get "/employees/999999"
      expect(response).to have_http_status(:not_found)
    end
  end
end